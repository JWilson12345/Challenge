...
import { auth, db, firebaseReady, friendlyError, storage } from "./lib/firebase";
...
function BottomNav() {
  const mobileItems = navItems.filter((item) => ["Home", "Current", "Leaderboard", "Overall", "Settings"].includes(item.label));
  return (
      ...
      <section className="challenge-header">
        <div>
          ...
          <dl className="meta-grid">
            <div>
              <dt>Starts</dt>
              <dd>{formatDate(challenge.startAt, { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</dd>
            </div>
            <div>
              <dt>Ends</dt>
              <dd>{formatDate(challenge.endAt, { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</dd>
            </div>
            <div>
              <dt>Scoring</dt>
...
function HistoricalChallengePage({ data }: { data: AppData }) {
  ...
  return (
    <Page title={challenge.title} eyebrow="Historical challenge">
      ...
      <Panel title="Historical submissions">
        <Feed submissions={data.submissions.filter((submission) => submission.challengeId === challenge.id)} data={data} />
      </Panel>
    </Page>
  );
...
function ProfilePage({ data }: { data: AppData }) {
  ...
  const save = async (event: FormEvent) => {
    event.preventDefault();
    if (!db || !storage || !data.firebaseUser) return;
    setBusy(true);
    try {
      let photoURL = data.currentPlayer?.photoURL ?? "";
      if (avatarFile) {
        const blob = await compressImage(avatarFile, 512, 0.82);
        const path = `profiles/${data.firebaseUser.uid}/avatar.jpg`;
        ...
        <form className="stack form-grid" onSubmit={save}>
          <div className="profile-preview">
            ...
            <label className="file-button">
              <ImagePlus size={18} /> Profile photo
          ...
          <button className="primary-action" disabled={busy}>{busy ? "Saving..." : "Save profile"}</button>
        </form>
      </Panel>
    </Page>
  );
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const fileRef = ref(storage, `submissions/${data.firebaseUser.uid}/${submissionRef.id}/${crypto.randomUUID()}.jpg`);
        await uploadBytes(fileRef, blob, { contentType: "image/jpeg" });
  ...
  return (
    <Modal title={submission ? "Edit submission" : "Submit result"} onClose={onClose}>
      <form className="stack" onSubmit={submit}>
        ...
        <button className="primary-action" disabled={busy}>{busy ? "Saving..." : "Save submission"}</button>
      </form>
    </Modal>
  );
}
  ...
  return (
    <article className="submission-card">
      ...
      <footer className="post-actions">
        <button className={hasVoted ? "active" : ""} onClick={() => toggleVote(data, "post", submission.id, 1)}>
          <ThumbsUp size={17} /> {postVotes.length}
        </button>
        {!condensed && (
          <a href={`#comments-${submission.id}`}><MessageCircle size={17} /> {data.comments.filter((comment) => comment.postId === submission.id).length}</a>
        )}
        {canManage && (
          <>
            <button onClick={() => setEditing(true)}><Pencil size={17} /> Edit</button>
            <button onClick={() => deleteSubmission(data, submission)}><Trash2 size={17} /> Delete</button>
          </>
        )}
      </footer>
...
function Leaderboard({ rows, currentUserId, compact = false, overall = false }: { rows: RankedPlayer[]; currentUserId?: string; compact?: boolean; overall?: boolean }) {
  ...
  return (
    <div className={compact ? "leaderboard compact" : "leaderboard"}>
      {rows.map((row) => (
        <div className={`leaderboard-row rank-${Math.min(row.rank, 3)} ${row.userId === currentUserId ? "current" : ""}`} key={row.userId}>
...
function Countdown({ challenge }: { challenge: Challenge }) {
  ...
  return (
    <div className="countdown" aria-live="polite">
      ...
      <strong>{formatCountdown(challenge.endAt, now)}</strong>
    </div>
  );
}
